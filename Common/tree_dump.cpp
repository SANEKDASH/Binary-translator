...
void EndTreeGraphDump()
{
    if (log_file == nullptr)
    {
        perror("EndTreeGraphDump() failed to write an info");
                         ...
                         const int line)
{
    ...
    fprintf(log_file, "DATE : %s \nTIME : %s\n"
                      ...
                      "Line: %d\n"
                      "<img height=\"700px\" src=\"graphdump%lu.svg\">\n"
                      "-----------------------------------------------------------------\n",
                      __DATE__,
                      __TIME__,
                      file,
                         ...
                         FILE     *dot_file)
{
    ...
    else if (node->type == kVarDecl)
    {
        LOG_PRINT("node%p [style = filled, fillcolor = \"orange\", shape = Mrecord, label = "
                  "\"VAR POS: %lu | type : VAR DECLARATION | {parent: %p | pointer: %p | left: %p | right: %p} \"]\n",
                  node,
                  node->data.variable_pos,
                  node->parent,
                  node,
    ...
    else if (node->type == kFuncDef)
    {
        LOG_PRINT("node%p [style = filled, fillcolor = \"white\", shape = Mrecord, label = "
                  "\"FUNC POS: %lu | type : FUNC DEF | {parent: %p | pointer: %p | left: %p | right: %p} \"]\n",
                  node,
                  node->data.variable_pos,
                  node->parent,
                  node,
